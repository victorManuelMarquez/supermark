...
def moduloProducto(app):
    ...
    ## al presionar 'Enter'
    tabla_productos.cuerpo.bind('<Return>', lambda key : evt.agregarAlCarrito(tabla_productos, tabla_carrito, total))

    ## doble clic con el ratón
    tabla_productos.cuerpo.bind('<Double-Button>', lambda mbtn : evt.agregarAlCarrito(tabla_productos, tabla_carrito, total))
...
def moduloOperaciones(app):
    ...
    btn_comprar = Button(master, text="Comprar", command=lambda: evt.realizarCompra(app, tabla_carrito))
    btn_comprar.pack(fill='x', expand=True, pady=6)

    btn_vaciar = Button(master, text="Vaciar\ncarrito", command=lambda: evt.vaciarCarrito(tabla_carrito))
